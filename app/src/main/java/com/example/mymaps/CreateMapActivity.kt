...
class CreateMapActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        val sv = LatLng(37.4, -122.2)
        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(sv, 10f))
    }

    private fun showAlertDialog(latLng: LatLng) {
        val placeFormView = LayoutInflater.from(this).inflate(R.layout.dialog_create_place, null)
        val dialog = AlertDialog.Builder(this).setTitle("Create a Marker").setView(placeFormView).setNegativeButton("Cancel", null).setPositiveButton("OK", null).show()


        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
            val title = placeFormView.findViewById<EditText>(R.id.etTitle).text.toString()
            val desc = placeFormView.findViewById<EditText>(R.id.etDescription).text.toString()
